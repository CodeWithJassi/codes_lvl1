...
int SpeedPin = 32; // speed pin from other joystick
// int Speed_Y = 39;


// REPLACE WITH THE MAC Address of your receiver 
uint8_t broadcastAddress[] = {0x08, 0xB6, 0x1F, 0x3B, 0x5E, 0x28}; //08:B6:1F:3B:5E:28
...
// Format structured data
  ...
  myData.cmd = cmd();
  myData.spid= map(speedraw, 0, 4095, 0,5);    // linearly maps (0, 4095) -> (0, 5) which will be using for speed control 
  Serial.println(x);
  Serial.println(y);
...
int digi(int analval){
    if ( analval < 1700)  return 0;
    if((analval  > 1700)and (analval< 2300))  return 1;
    if (analval > 2300) return 2;
    else return 3;
}
